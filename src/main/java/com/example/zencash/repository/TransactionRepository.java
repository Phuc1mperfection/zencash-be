...
public interface TransactionRepository extends JpaRepository<Transaction, Long> {
        ...
        List<CategoryStatisticResponse> getCategoryStatisticsByType(@Param("user") User user,
                        @Param("type") String type);

        @Query(value = "SELECT NEW com.example.zencash.dto.CategoryStatisticResponse(" +
                        "t.category.id, t.category.name, SUM(t.amount), COUNT(t)) " +
                        "FROM Transaction t " +
                        "WHERE t.budget.user = :user AND t.type = :type AND t.budget.id = :budgetId " +
                        "GROUP BY t.category.id, t.category.name")
        List<CategoryStatisticResponse> getCategoryStatisticsByTypeAndBudget(
